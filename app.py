...
SECTION_HEADERS = {
    "VALIDITY": "Validity",
    "ACADEMICS": "Academic Skills",
    "INTELLECTUAL FUNCTIONING": "General Cognitive Ability",
    "ATTENTION/EXECUTIVE": "Attention/Executive",
...
def merge_with_lookup(df, lookup_df):
    ...
    df['scale_clean'] = df['scale'].apply(normalize_scale)
    lookup = lookup_df.copy()
    lookup['scale_clean'] = lookup['scale'].apply(normalize_scale)
...
def server(input, output, session):
    ...
    def lookup():
        ...
        p = Path("~/Dropbox/neuropsych_lookup_table.csv")
